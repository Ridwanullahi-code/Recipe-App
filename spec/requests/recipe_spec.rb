...
RSpec.describe Recipe, type: :model do
  ...
  let!(:flour) { Food.create(name: 'Flour', price: 0.50, measurement: 'gram', user: user) }
  let!(:sugar) { Food.create(name: 'Sugar', price: 0.75, measurement: 'gram', user: user) }
  let!(:apple) { Food.create(name: 'Apple', price: 5, measurement: 'gram', user: user) }
